...
def to_uppercase(df):
    for column in df.columns:
        if df[column].dtype == "object":
            df[column] = df[column].apply(lambda x: x.upper().strip() if isinstance(x, str) else x)
    return df
...
def Process_Tresholds_Columns(df, column_name, sep, threshold):
    ...
    formatted_values = formatted_values.where(
        formatted_values.isin(value_counts.index), "ALTRO" if column_name != "AOP" else "A"
    )
    ...
    for index, row in df.iterrows():
        ...
        for value in values_list:
            ...
            if value in value_counts.index:
                ...
                other_col_name = f"{column_name}_ALTRO" if column_name != "AOP" else f"{column_name}_A"
...
def Preprocess_Dataset(df):
    ...
    df.loc[df['ETA_DIAGNOSI_MIN'] == 'ND', ['ETA_DIAGNOSI_MIN', 'ETA_DIAGNOSI_MAX']] = df.loc[df['ETA_DIAGNOSI_MIN'] == 'ND', 'ETA']
...
def Create_Pipeline():
    ...
    preprocess_pipeline = Pipeline(
        [("preprocess", preprocess), ("onehot", onehot), ("drop_cols", drop_cols)]
    )
